...
#from pipeline_eds.cli import GLOBAL_SHUTDOWN_EVENT # MOVE THIS TO A DEDICATED INTERUPT FILE # actually the interrupt would come from the program that consumes this library
...
def browser_get_input(manager: Any, message: str, hide: bool, stop_event: threading.Event | None = None) -> str | None:
    req_id = manager.register_prompt("input_key", message, hide)
    url = manager.get_server_url()

    if not is_server_running(url):
        from .server import run_prompt_server_in_thread
        run_prompt_server_in_thread() # Starts on default 8083
        url = manager.get_server_url()

    encoded_msg = urllib.parse.quote_plus(message)
    full_url = f"{url}/config_modal?request_id={req_id}&message={encoded_msg}&hide_input={hide}"
...
def browser_get_input_(manager: Any, key: str, prompt_message: str, hide_input: bool) -> str | None:
    """
